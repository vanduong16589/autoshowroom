...
if ($saveOrder)
{
    $saveOrderingUrl = TZ_Portfolio_PlusHelperAddon_Datas::getRootURL($addonId)
        .'&addon_task=currencies.saveOrderAjax&tmpl=component';
    ...
    <?php }?>
        ...
        <table class="table table-striped"  id="addonDataList">
            ...
            <tr>
                ...
                <th class="nowrap">
                    <?php echo JHtml::_('grid.sort','JDEFAULT','value.default',$listDirn,$listOrder);?>
                </th>
                <th class="nowrap">
                    <?php echo JHtml::_('grid.sort','JGLOBAL_FIELDSET_DISPLAY_OPTIONS','value.display',$listDirn,$listOrder);?>
                </th>
                <th width="3%" class="nowrap center">
                    <?php echo JHtml::_('grid.sort','Code','value.code',$listDirn,$listOrder);?>
                </th>
                <th width="3%" class="nowrap center">
                    <?php echo JHtml::_('grid.sort','Sign','value.sign',$listDirn,$listOrder);?>
                </th>
            ...
            <tr class="row<?php echo $i % 2; ?>" sortable-group-id="<?php echo $data -> extension_id;?>">
                ...
                <td class="center">
                    <div class="btn-group">
                        <?php echo JHtml::_('jgrid.published', $data->published, $i, 'currencies.', true, 'cb'); ?>
                    </div>
                </td>
                <td>
                    <?php if ($canEdit || $canEditOwn) : ?>
                        <a href="<?php echo JRoute::_(TZ_Portfolio_PlusHelperAddon_Datas::getRootURL($addonId)
                            .'&addon_task=currency.edit&id=' . (int) $data->id); ?>">
                ...
                <td width="1%">
                    <?php if (isset($item->default) && ($item->default == '0' || $item->default == '1')):?>
                        <?php echo JHtml::_('jgrid.isdefault', $item->default != 0, $i, 'currencies.', $item->default != '1');?>
                    <?php endif;?>
                </td>
                <td width="1%">
                    <?php if ($item -> display = 1):
                        echo JHtmlJGrid::action($i,'code','currencies.',$item -> sign,'test','test2',true);
                    else:
                        echo JHtmlJGrid::action($i,'currencies.sign','',$item -> code);
                    endif;?>
                </td>
