...
class PlgTZ_Portfolio_PlusMediaTypeAudioViewArticle extends JViewLegacy{
    protected $item     = null;
        ...
        if($item){
            if($media = $item -> media){
                if(isset($media -> audio)){
                    ...
                    if($rsize = $params -> get('mt_audio_related_thumb_size','o')){
                        if(isset($audio -> thumbnail) && !empty($audio -> thumbnail)) {
                            $image_url_ext      = JFile::getExt($audio->thumbnail);
                            $image_url          = str_replace('.' . $image_url_ext, '_' . $rsize . '.'
                                . $image_url_ext, $audio->thumbnail);
                            $audio -> related_thumbnail = JURI::root().$image_url;
                        }
                    }
                    ...
                    if($size = $params -> get('mt_audio_thumbnail_size','o')){
                        if(isset($audio -> thumbnail) && !empty($audio -> thumbnail)) {
                            ...
                            if($this -> getLayout() != 'related') {
                                JFactory::getDocument()->addCustomTag('<meta property="og:image" content="' . $audio->thumbnail . '"/>');
                                if($author = $item -> author_info){
                                    JFactory::getDocument() -> setMetaData('twitter:image',$audio->thumbnail);
                                }
                            }
                        }
                    }
                    ...
                    if(isset($audio -> type) && $audio -> type == 'soundcloud'){
                        if(isset($audio -> code) && !empty($audio -> code)) {
                            ...
                            if($audioWidth   = $params -> get('mt_audio_soundcloud_width','100%')){
                                if(!preg_match('/[0-9]+(\%|px)/i',$audioWidth)){
                                    $audioWidth .= 'px';
                                    $params -> set('mt_audio_soundcloud_width',$audioWidth);
                                }
                            }
                            if($audioHeight   = $params -> get('mt_audio_soundcloud_height')){
                                if(!preg_match('/[0-9]+(\%|px)/i',$audioHeight)){
                                    $audioHeight .= 'px';
                                    $params -> set('mt_audio_soundcloud_height',$audioHeight);
                                }
                            }
